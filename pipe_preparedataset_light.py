...
list_dir=r"path/to/your/images/folder"
image_dir=os.path.join(list_dir,'image')
...
for img_name in os.listdir(image_dir):
    ...
    idx=int(img_name[-7:-4])
    profile_dir = os.path.join(list_dir, 'light_detected')
    os.makedirs(os.path.join(list_dir,'fit_image',f'{idx:02d}'),exist_ok=True)
    #inital rgb image
    rgb_image = np.zeros((height, width, 3))
    # read centers of light source
    center_path=os.path.join(list_dir,'light_detected',f'cam{idx:03d}_light_center.txt')
    centers=np.loadtxt(center_path)
    # note that simulated data have three light source
    for light_idx in range(3):
        centery, centerx =centers[light_idx] #BGR
        # For each light, we need to fit a curve each channel(rgb)
        for rgb_idx in range(3):
            profile_name=img_name.split('.')[0]+f'_light{(light_idx+1):03d}_{profilelist[rgb_idx]}'
            ...
            center_value = data[0]
            flipped_data = np.flip(data[1:])  # 翻转除了第一个数据外的所有数据
            ...
            bounds = (
                ...
                [1000 * np.max(intensity_data), np.inf, 1, 1, 0.2]  # 最大边界
            )
        ...
        output_image.save(os.path.join(list_dir,'fit_image',f'{idx:02d}',f"cam{idx:02d}_{bulb_list[light_idx]}.png"))
