...
import math
def prune_by_gradients(viewpoint_cameras, pc : GaussianModel, pipe, bg_color : torch.Tensor,centers, scaling_modifier = 1.0, separate_sh = False, override_color = None, use_trained_exp=False):
    frame_idx = 0
    ...
    for viewpoint_camera in viewpoint_cameras:
        ...
        light_center = centers[(viewpoint_camera.colmap_id - 1) * 3:viewpoint_camera.colmap_id * 3, :]
        light_centers = light_center.repeat_interleave(3, 0)
        ...
        target = rendered_image[2] * light_mask[2]
