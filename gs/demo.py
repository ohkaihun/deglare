...
def load_checkpoint(checkpoint: str, data_dir: str, rasterizer: Literal["original", "gsplat"] = "original",
                    data_factor: int = 1):
...
def test_proper_pruning(splats, splats_after_pruning):
    ...
    print("Report {}% pruned, max pixel error = {}, total pixel error = {}".format(percentage_pruned, max_pixel_error,
                                                                                   total_error))


def get_mask3d(splats, prompt, data_dir: str, results_dir: str, show_visual_feedback: bool = False,
               mask_interval: int = 1, voting_method: Literal["gradient", "binary", "projection"] = "gradient",
               mask_dir=None):
    ...
    with torch.autocast("cuda", dtype=torch.bfloat16):
        ...
        colors = colors_dc[:, 0,
                 :] * 0  # Just to show that gradient (opacity * transmittance) is independent of color. Any value will work.
        ...
        for image, (frame_idx, object_ids, masks) in zip(
            ...
            if voting_method == "gradient":
                ...
                means2d_mask = (means2d[:, 0] >= 0) & (means2d[:, 0] < width) & (means2d[:, 1] >= 0) & (
                            means2d[:, 1] < height)
...
def render_to_gif(output_path: str, splats, feedback: bool = False, use_checkerboard_background: bool = False,
                  no_sh: bool = False):
    if feedback:
        cv2.destroyAllWindows()
...
def main(
        data_dir: str = r"C:\Users\Mayn\work\2dgs_water\ray\real_scene\0310", # colmap path
        checkpoint: str = r"C:\Users\Mayn\work\2dgs_water\gaussian-splatting\output\light_gs_del\chkpnt15000.pth", # checkpoint path, can generate from original 3DGS repo
        prompt: str = "chair", # prompt
        results_dir: str = ".\output/delete_310", # output path
